...
/**
  ...
  * @brief: 将SPI初始化为默认配置。
  *
  * @param: SPIx,使用哪组SPI；STAR_SPI0或STAR_SPI1。
  * 
  * @retVal: void
  */
...
{	
	...
	SPIx->SSPCPSR = SPI_InitStruct->SPI_ClkPreDiv;
}
...
void SPI_ReceiveDataNum_Config(SPI_TypeDef* SPIx, uint32_t RxNum)
{
	...
	SPIx->SSP_RX_CNT_LSB =  (RxNum&0xffff);
	SPIx->SSP_RX_CNT_MSB =  (RxNum>>16);
}
...
{
	assert_param(IS_SPI_ALL_PERIPH(SSPx));
	assert_param(IS_SPI_INT_TYPE(SPI_INT));
	
	SPIx->SSPICR |= (uint16_t)(SPI_INT&0x0003);
}
...
void SPI_DMA_Cmd(SPI_TypeDef* SPIx,uint16_t SPI_DMA_TxRx,FunctionalState NewState)
{
	...
	if(NewState != DISABLE)
	{
		SPIx->SSPDMACR |= SPI_DMA_TxRx;
	}
	else
	{
		SPIx->SSPDMACR &= (uint16_t)(~SPI_DMA_TxRx);
	}
}
