...
void I2C_SoftwareResetCmd(I2C_TypeDef *I2Cx, FunctionalState NewState)
{
	...
	if (NewState != DISABLE)
	{
		I2Cx->CONTROL |= (1ul<<I2C_SWRST_Pos);
	}
	else
	{
		I2Cx->CONTROL &= ~(1ul<<I2C_SWRST_Pos);
...
{	
	...
	if(NewState != DISABLE)
	{
		I2Cx->CONTROL  |= I2C_START_SET_Msk;
		while ((I2Cx->STATUS1 & STATUS1_START_BIT_DET) != 1);
	}
	else
	{
		I2Cx->CONTROL &= (~I2C_START_SET_Msk);
...
{	
    ...
    if (NewState != DISABLE)
    {
        I2Cx->CONTROL |= I2C_STOP_SET_Msk;
		while ((I2Cx-> STATUS1 & STATUS1_STOP_BIT_DET) == 0);  //stop bit detect
    }
    else
    {
        I2Cx->CONTROL &= (~I2C_STOP_SET_Msk);
...
{	
	...
	I2Cx->CONTROL &= (~I2C_TX_NUM_Msk);
	I2Cx->CONTROL |= ((Num&0x0f) << I2C_TX_NUM_Pos );
}
...
void I2C_WaitWriteDataFinish(I2C_TypeDef *I2Cx)
{
	...
	while((I2Cx->STATUS1 & STATUS1_TX_FIFO_EMPTY)==0);	//wait until tx fifo is empty
	while ((I2Cx->STATUS1 & STATUS1_TC_FLAG)==0);		//transmit complete flag
}
...
{	
	assert_param(IS_I2C_ALL_PERIPH(I2Cx));
	while ((I2Cx->STATUS1 & STATUS1_RX_FIFO_EMPTY));
}
...
/**
  ...
  *			可设置的值包括:
  *				STATUS1_START_BIT_DET - 起始位信号被检测; 
  *				STATUS1_ADDR_7BIT_MATch - 地址7位发送/接收匹配;	STATUS1_ADDR_10BIT_MAth - 地址10位发送/接收匹配；
  *				STATUS1_TX_FIFO_FULL - 发送FIFO已满;	STATUS1_TX_FIFO_EMPTY - 发送FIFO已空；
  *				STATUS1_RX_FIFO_FULL - 接收FIFO已满;	STATUS1_RX_FIFO_EMPTY - 接收FIFO已空；
  *				STATUS1_STOP_BIT_DET - 停止位信号被检测; STATUS1_TC_FLAG - 发送完成；
  *				STATUS1_DATA_RECEIVED - 数据到达。
  *						
  * 
...
{
	assert_param(IS_I2C_ALL_PERIPH(I2Cx));
	while((I2Cx->STATUS2 & STATUS2_BUS_BUSY));
}
...
{
	assert_param(IS_I2C_ALL_PERIPH(I2Cx));
	while((I2Cx->STATUS1 & STATUS1_TX_FIFO_FULL));
}
...
FlagStatus I2C_CheckACKIsFail(I2C_TypeDef *I2Cx)
{
	...
	if(I2Cx->STATUS2 & STATUS2_ACK_FAILURE)
	{
		isFail= SET;
	}
	else
...
void I2C_TxInterrupt_TxbufferLevel_Config(I2C_TypeDef *I2Cx, uint8_t TxbufferWlevel)
{
	...
	if(TxbufferWlevel <4)
	{
		I2Cx->CONTROL &= ~I2C_TX_BUFFER_LEVEL_Msk;
		I2Cx->CONTROL  |= (TxbufferWlevel<<I2C_TX_BUFFER_LEVEL_Pos);
	}
}
...
void I2C_RxInterrupt_RxbufferLevel_Config(I2C_TypeDef *I2Cx, uint8_t RxbufferWlevel)
{
	...
	if(RxbufferWlevel<4)
	{
		I2Cx->CONTROL &= ~I2C_RX_BUFFER_LEVEL_Msk;
		I2Cx->CONTROL |= ( RxbufferWlevel << I2C_RX_BUFFER_LEVEL_Pos);
	}
}
