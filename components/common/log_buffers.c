...
static void print_buffer(const char *tag, const char *buffer, uint16_t buff_len, log_level_t log_level, char *output_str, print_line_t print_line_func);
...
static void print_buffer(const char *tag, const char *buffer, uint16_t buff_len, log_level_t log_level, char *output_str, print_line_t print_line_func)
{
    ...
    do {
        ...
        XS_LOG_LEVEL(log_level, tag, "%s", output_str);
...
static void log_buffer_hex_line(uintptr_t orig_buff, const void *ptr_line, char *output_str, int buff_len)
{
    ...
    for (int i = 0; i < buff_len; i++) {
        output_str += log_util_cvt_hex(ptr[i], 2, output_str);
        *output_str++ = ' ';
    }
    *--output_str = 0;
}
...
static void log_buffer_hexdump_line(uintptr_t orig_buff, const void *ptr_line, char *output_str, int buff_len)
{
    ...
    *output_str++ = '0';
    *output_str++ = 'x';
    output_str += log_util_cvt_hex(orig_buff, sizeof(uintptr_t), output_str);
    *output_str++ = ' ';
