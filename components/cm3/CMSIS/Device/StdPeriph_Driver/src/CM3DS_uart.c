...
/**
  ...
  * @brief: 将UART初始化为默认配置。
  *
  * @param: UARTx,使用哪组UART;CM3有2组UART:CM3DS_MPS2_UART0和CM3DS_MPS2_UART1。
  * 
  * @retVal: void
  */
...
/**
  ...
  *						SET - 产生了UART_Interrupt指定中断类型的中断；RESET - 未产生UART_Interrupt指定中断类型的中断。
  */
...
void UART_SendData(UART_TypeDef *UARTx, uint8_t Data)
{
	...
	while ( (UARTx->STATE & CM3DS_MPS2_UART_STATE_TXBF_Msk));
...
uint8_t  UART_ReceiveData(UART_TypeDef *UARTx)
{
	...
	/* Receive Data */
	while((UARTx->STATE & CM3DS_MPS2_UART_STATE_RXBF_Msk) == 0);
